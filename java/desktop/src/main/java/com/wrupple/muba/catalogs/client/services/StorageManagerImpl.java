...
import com.wrupple.muba.desktop.client.event.EntriesDeletedEventHandler;
import com.wrupple.muba.desktop.client.event.EntryCreatedEvent;
import com.wrupple.muba.desktop.client.event.EntryCreatedEventHandler;
import com.wrupple.muba.desktop.client.event.EntryUpdatedEvent;
import com.wrupple.muba.desktop.client.event.EntryUpdatedEventHandler;
import com.wrupple.muba.desktop.client.services.logic.impl.VisitCatalogNeighbors;
...
public class StorageManagerImpl implements StorageManager {
	...
	class InvalidateCache implements EntryCreatedEventHandler, EntryUpdatedEventHandler, EntriesDeletedEventHandler {
		...
		public void onEntryUpdated(EntryUpdatedEvent e) {
			if (CatalogDescriptor.CATALOG_ID.equals(e.entry.getCatalog()) || FieldDescriptor.CATALOG_ID.equals(e.entry.getCatalog())) {
				clearCache(e.host, e.domain);
			}
		}
		...
		public void onEntryCreated(EntryCreatedEvent e) {
			if (CatalogDescriptor.CATALOG_ID.equals(e.entry.getCatalog())) {
	...
	class LoadCatalogDescriptor extends DataCallback<JsCatalogDescriptor> {
	...
	public CatalogDescriptor loadFromCache(String host, String domain, String catalog) {
		...
		if (regreso == null) {
			GWT.log("Catalog Descriptor with id " + catalog + " not found in cache, expect subsequent errors");
		}
		return regreso;
	}
